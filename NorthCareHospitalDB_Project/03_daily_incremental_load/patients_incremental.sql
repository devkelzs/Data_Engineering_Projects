CREATE OR ALTER PROCEDURE etl.usp_load_patients_incremental
    @batch_id INT
AS 
BEGIN
    SET NOCOUNT ON;

    DECLARE @records_read INT;
    DECLARE @records_staged INT;

    --COUNT RECORDS READ FROM RAW 
    SELECT @records_read = COUNT(*)
    FROM raw.Patients;
...
SET @records_staged = @@ROWCOUNT;

UPDATE monitoring.etl_batch_log
SET records_staged = @records_staged
WHERE batch_id = @batch_id;
