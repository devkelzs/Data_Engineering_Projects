--Initial full load from raw.patients into silver.patients
/*
		  ...
		  'SHA2_256',
		   CAST(
			  CONCAT(
					...
					COALESCE(CONVERT(VARCHAR(19), registration_date, 120), ''), '|',
					COALESCE(TRIM(notes), '')
			 ) AS VARCHAR(1000)
    )
) AS record_hash
...
(
    SELECT
        ...
        CASE
            WHEN s.patient_id IS NULL
                THEN 'NEW'

            WHEN f.record_hash <> s.record_hash
                THEN 'CHANGED'
...
WHERE passes_final_validation = 1;


--Initial full load into quarantine.patients
...
SELECT
    ...
    last_name,
    gender,
    CONVERT(VARCHAR(50), date_of_birth, 23),
    email,
    phone,
    city,
    blood_group,
    CONVERT(VARCHAR(50), registration_date, 120),
    quarantine_reason,
    CAST(GETDATE() AS DATETIME2(0))

FROM final_validation

WHERE passes_final_validation = 0;
